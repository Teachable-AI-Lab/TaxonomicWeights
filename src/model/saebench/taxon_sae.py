...
class LinearTaxonStage(nn.Module):
    ...
    with ``nn.Linear`` → ``ReLU`` and operates on 2-D ``[B, d_model]`` tensors.
    ...
    def _pairwise_log_softmax(
        ...
        """Pairwise softmax over sibling pairs. Input/output shape: [B, C]."""
        B, C = logits.shape
    ...
    def forward(
        ...
        all_logits = F.relu(self.linear(x))  # [B, total_out_channels]
