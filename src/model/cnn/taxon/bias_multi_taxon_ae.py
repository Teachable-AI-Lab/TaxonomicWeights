"""Sigmoid + Bias (Loss-Free Balancing) multi-hierarchy taxon autoencoder.

Composes :class:`~.encoder.BiasMultiTaxonResNetEncoder` with
:class:`~.decoder.TaxonResNetDecoder`.

Each encoder stage applies K independent sigmoid+bias taxonomy hierarchies
with a sigmoid+bias inter-hierarchy gate.  Forward output:

* ``reconstruction``
* optional ``details`` dictionary

**No regularisation terms at all** — diversity is maintained purely through
the bias update rule at both the leaf and inter-hierarchy gate levels.
...
from .encoder import BiasMultiTaxonResNetEncoder


class BiasMultiTaxonAutoencoder(nn.Module):
    """ResNet-style autoencoder with K sigmoid+bias taxonomy hierarchies per stage."""
    ...
    def __init__(
        ...
        self.encoder = BiasMultiTaxonResNetEncoder(
...
__all__ = ["BiasMultiTaxonAutoencoder"]
