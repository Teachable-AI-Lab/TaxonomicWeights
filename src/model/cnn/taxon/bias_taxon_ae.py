"""Per-Node Bias (Loss-Free Balancing) taxon autoencoder wrapper.
...
:class:`~.taxon_ae.TaxonAutoencoder`.  Instead of DKL / entropy losses,
non-differentiable per-node biases are added to the softmax inputs to
heuristically balance node usage.  Forward output:
...
class BiasTaxonAutoencoder(nn.Module):
    """ResNet-style taxonomic autoencoder with hierarchical routing + per-node bias."""
