...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Test SparseConvAutoencoder architecture and sparsity behaviour"
    )
    ...
    parser.add_argument(
        "--stage-channels", type=int, nargs=4, default=[64, 128, 256, 512]
    )
    parser.add_argument("--stage-strides", type=int, nargs=4, default=[1, 2, 2, 2])
    parser.add_argument("--sparsity-type", type=str, default="l1", choices=["l1", "kl"])
    parser.add_argument("--sparsity-target", type=float, default=0.05)
    parser.add_argument("--latent-activation", type=str, default="relu", choices=["relu", "sigmoid"])
    parser.add_argument(
        "--use-stem-maxpool",
...
def print_encoder_structure(model: SparseConvAutoencoder) -> None:
    print("\n== SAE Encoder Stage Structure ==")
    for idx, stage in enumerate(model.encoder.stages, start=1):
        n_blocks = len(stage.blocks)
        first_block = stage.blocks[0]
        stride = first_block.main[1].stride[0] if hasattr(first_block.main[1], "stride") else "?"
        print(
            f"stage{idx}: blocks={n_blocks}  "
            f"in_ch={model.encoder.stage_input_channels[idx - 1]}  "
            f"out_ch={model.encoder.stage_channels[idx - 1]}"
        )
    print(f"latent_channels={model.encoder.final_channels}  "
          f"latent_activation={model.encoder.latent_activation}  "
          f"sparsity_type={model.encoder.sparsity_type}")


def check_sparsity(model: SparseConvAutoencoder, x1: torch.Tensor, x2: torch.Tensor) -> None:
    """Verify that sparsity values are finite and differ across batches."""
    print("\n== Sparsity Checks ==")
    model.eval()
    with torch.no_grad():
        _, sp1 = model(x1)
    ...
    print(f"batch1 sparsity={float(sp1):.6f}")
    print(f"batch2 sparsity={float(sp2):.6f}")
...
def check_latent_sparsity_level(
    model: SparseConvAutoencoder, x: torch.Tensor
) -> None:
    """Report fraction of latent features that are exactly zero (ReLU) or near-zero."""
    print("\n== Latent Sparsity Level ==")
    model.eval()
    with torch.no_grad():
    ...
    print(f"latent_min={latent.min().item():.5f}  latent_max={latent.max().item():.5f}")
    print(f"latent_mean={latent.mean().item():.5f}  latent_std={latent.std().item():.5f}")


def print_architecture_diagram(model: SparseConvAutoencoder, details: dict) -> None:
...
def main() -> None:
    ...
    print(f"sparsity_type={args.sparsity_type}  sparsity={float(sparsity):.6f}")
