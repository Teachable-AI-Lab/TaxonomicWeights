    ...
    if variant == 'topk':
        model = TopKMultiTaxonAutoencoder(
            **common_kw,
            ...
            use_gate_value=mc.get("use_gate_value", ckpt_args.get("use_gate_value", False)),
        )
    elif variant == 'bias':
...
def analyze_skip_decomposition(
    ...
    """Legacy placeholder kept for call-site compatibility after skip removal."""
    del model, data_loader, device, save_dir, num_images, num_sets
    print("  B1 legacy decomposition removed after skip-path retirement.")
