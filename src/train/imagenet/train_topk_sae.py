#!/usr/bin/env python3
"""Train TopKSparseConvAutoencoder on ImageNet-1k.

Mirrors src/train/train_sae_imagenet.py.  Sparsity is controlled exclusively
...
def save_training_curves(history: dict, output_dir: Path) -> None:
    ...
    plt.suptitle(f"TopK SAE ImageNet-1k training curves  (k={history.get('topk_k', '?')})",
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train TopK SAE on ImageNet-1k")
    parser.add_argument("--config",          type=str,   default="")
    parser.add_argument("--data-root",       type=str,   default=d.get("data_root",       "./data/imagenet"))
    parser.add_argument("--output-dir",      type=str,   default=o.get("output_dir",      "./outputs/sae_topk_imagenet_r18"))
...
def main() -> None:
    ...
    print(
        "Training setup (TopK SAE ImageNet-1k):\n"
