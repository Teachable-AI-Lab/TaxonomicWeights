#!/usr/bin/env python3
"""Train BottleneckTopKTaxonAutoencoder on ImageNet-1k.

Plain ResNet stages followed by a single TopK taxon stage at the bottleneck
(single hierarchy, hierarchical pairwise softmax + TopK + AuxK revival).

Loss = recon + auxk_w*auxk + dkl_w*dkl
"""
...
def save_training_curves(history, output_dir) -> None:
    ...
    plt.suptitle("Bottleneck TopK Taxon AE — ImageNet")
    plt.tight_layout()
...
def parse_args():
    ...
    p.add_argument("--output-dir", type=str,
                   default=o.get("output_dir", "./outputs/bottleneck_topk_taxon_ae_imagenet"))
...
def main() -> None:
    ...
    kl = int(_mc.get("k_leaves", 0))
    kl_tag = f"_kl{kl}" if kl > 1 else ""
    run_suffix = f"_L{L}{kl_tag}"
    ...
    print(f"Bottleneck TopK Taxon AE (ImageNet): device={device} n_params={n_params:,} L={L} "
          f"k_leaves={kl} latent_ch={model.encoder.final_channels} "
