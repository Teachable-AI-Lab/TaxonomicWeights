#!/usr/bin/env python3
"""Train BaselineConvAutoencoder on ImageNet-1k (ResNet-18 stage layout).

Mirrors src/train/train_baseline_ae_celeba_hq.py but uses ImageNet1kHFLoader
(224×224, stem_stride=2, stem max-pool).
...
def save_training_curves(history: dict, output_dir: Path) -> None:
    ...
    plt.suptitle("Baseline AE ImageNet-1k training curves", fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train BaselineConvAutoencoder on ImageNet-1k")
    parser.add_argument("--config",         type=str,   default="")
    parser.add_argument("--data-root",      type=str,   default=d.get("data_root", "./data/imagenet"))
    parser.add_argument("--output-dir",     type=str,   default=o.get("output_dir",  "./outputs/imagenet/baseline_ae_imagenet"))
...
def main() -> None:
    ...
    print(
        "Training setup (Baseline AE ImageNet-1k):\n"
        f"  device={device}\n"
