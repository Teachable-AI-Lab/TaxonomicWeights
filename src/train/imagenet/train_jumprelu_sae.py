#!/usr/bin/env python3
"""Train JumpReLUSparseConvAutoencoder on ImageNet-1k.

Mirrors src/train/train_sae_imagenet.py.  Sparsity is controlled by a
...
def save_training_curves(history: dict, output_dir: Path) -> None:
    ...
    plt.suptitle(
        f"JumpReLU SAE ImageNet-1k training  (target_l0={history.get('target_l0', '?')})",
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train JumpReLU SAE on ImageNet-1k")
    parser.add_argument("--config",           type=str,   default="")
    parser.add_argument("--data-root",        type=str,   default=d.get("data_root",        "./data/imagenet"))
    parser.add_argument("--output-dir",       type=str,   default=o.get("output_dir",       "./outputs/sae_jumprelu_imagenet_r18"))
...
def main() -> None:
    ...
    print(
        "Training setup (JumpReLU SAE ImageNet-1k):\n"
