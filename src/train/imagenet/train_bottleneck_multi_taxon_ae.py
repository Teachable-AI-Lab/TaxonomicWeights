#!/usr/bin/env python3
"""Train BottleneckMultiTaxonAutoencoder on ImageNet-1k.
...
def save_training_curves(history, output_dir) -> None:
    ...
    plt.suptitle("Bottleneck Multi-Taxon AE — ImageNet")
    plt.tight_layout()
...
def parse_args():
    ...
    p.add_argument("--output-dir", type=str,
                   default=o.get("output_dir", "./outputs/bottleneck_multi_taxon_ae_imagenet"))
...
def main() -> None:
    ...
    print(f"Bottleneck Multi-Taxon AE (ImageNet): device={device} n_params={n_params:,} K={K} L={L} "
          f"hard={args.hard} latent_ch={model.encoder.final_channels} "
