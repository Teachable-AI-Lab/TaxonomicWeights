#!/usr/bin/env python3
"""Train MatryoshkaBatchTopKSparseConvAutoencoder on ImageNet-1k.

Matryoshka Batch-TopK SAE training:
  - A single encoder + decoder trained simultaneously at multiple sparsity
    levels (k_values, descending).
  - **Batch-TopK**: shared global threshold across the whole batch, enabling
    adaptive feature allocation per spatial region.
  - **Matryoshka loss**: weighted sum of per-level MSEs:
        total = Σ_i loss_weights[i] * MSE(recon_i, x)
              + sparsity_weight * aux_loss
  - AuxK auxiliary loss keeps dead latent features alive (computed at largest k).

Output directory suffix: _k<k1>-<k2>-..._sw<sparsity_weight>
  e.g.  sae_matryoshka_batch_topk_imagenet_r18_k128-64-32_sw1e-02/
"""
...
def save_recon_preview(
    ...
    with torch.no_grad():
        recon, _ = model(images)        # uses largest k
...
def save_training_curves(history: dict, output_dir: Path) -> None:
    ...
    plt.suptitle(
        f"Matryoshka Batch-TopK SAE ImageNet-1k  (k={k_str})",
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train Matryoshka Batch-TopK SAE on ImageNet-1k"
    )
    parser.add_argument("--config",              type=str,   default="")
    parser.add_argument("--data-root",           type=str,   default=d.get("data_root",           "./data/imagenet"))
    parser.add_argument("--output-dir",          type=str,   default=o.get("output_dir",          "./outputs/imagenet/sae_matryoshka_batch_topk_imagenet_r18"))
    ...
    parser.add_argument("--max-train-samples",   type=int,   default=d.get("max_train_samples",   100000))
    parser.add_argument("--max-val-samples",     type=int,   default=d.get("max_val_samples",     10000))
    ...
    parser.add_argument("--k-values",            type=int,   nargs="+",
                        default=m.get("k_values",        [128, 64, 32]))
...
def main() -> None:
    ...
    k_values = sorted(args.k_values, reverse=True)
    k_str    = "-".join(str(k) for k in k_values)
    ...
    k_aux = args.k_aux if args.k_aux is not None else k_values[0]
    ...
    print(
        "Training setup (Matryoshka Batch-TopK SAE ImageNet-1k):\n"
        f"  device={device}  k_values={k_values}  loss_weights={loss_weights}\n"
        f"  sparsity_weight={args.sparsity_weight}\n"
        f"  train_size={len(loader_obj.trainset)}  val_size={len(loader_obj.valset)}\n"
    ...
    _args_dict = {
        "model_variant":    "matryoshka_batch_topk",
        "in_channels":      3,
        "resnet_variant":   args.resnet_variant,
