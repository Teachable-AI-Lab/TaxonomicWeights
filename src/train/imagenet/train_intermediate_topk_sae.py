#!/usr/bin/env python3
"""Train IntermediateTopKSparseConvAutoencoder on ImageNet-1k.
...
def save_training_curves(history: dict, output_dir: Path) -> None:
    ...
    plt.suptitle(f"Intermediate Per-Position TopK SAE — ImageNet  (k per stage = {k_str})",
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Train Intermediate Per-Position TopK SAE on ImageNet")
    p.add_argument("--config",           type=str,   default="")
    p.add_argument("--data-root",        type=str,   default=d.get("data_root",       "./data/imagenet"))
    p.add_argument("--output-dir",       type=str,   default=o.get("output_dir",      "./outputs/imagenet/sae_intermediate_topk_imagenet_r18"))
...
def main() -> None:
    ...
    print(
        "Training setup (Intermediate Per-Position TopK SAE — ImageNet):\n"
