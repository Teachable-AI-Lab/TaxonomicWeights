#!/usr/bin/env python3
"""Train IntermediateMatryoshkaBatchTopKSparseConvAutoencoder on ImageNet-1k.
...
def save_training_curves(history: dict, output_dir: Path) -> None:
    ...
    plt.suptitle(
        f"Intermediate Matryoshka Batch-TopK SAE — ImageNet  (k per stage = {k_str})",
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train Intermediate-Stage Matryoshka Batch-TopK SAE on ImageNet"
    )
    parser.add_argument("--config",          type=str,   default="")
    parser.add_argument("--data-root",       type=str,   default=d.get("data_root",       "./data/imagenet"))
    parser.add_argument("--output-dir",      type=str,   default=o.get("output_dir",      "./outputs/imagenet/sae_matryoshka_intermediate_topk_imagenet_r18"))
    ...
    parser.add_argument("--max-train-samples", type=int, default=d.get("max_train_samples", None))
    parser.add_argument("--max-val-samples", type=int,   default=d.get("max_val_samples",  None))
...
def main() -> None:
    ...
    print(
        "Training setup (Intermediate Matryoshka Batch-TopK SAE — ImageNet):\n"
