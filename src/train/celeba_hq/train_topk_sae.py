#!/usr/bin/env python3
"""Train TopKSparseConvAutoencoder on CelebA-HQ.

Mirrors src/train/train_sae_celeba_hq.py.  Sparsity is controlled by
``topk_k``.  The optional AuxK auxiliary loss is weighted by
``sparsity_weight``.
...
def save_training_curves(history: dict, output_dir: Path) -> None:
    ...
    plt.suptitle(f"TopK SAE CelebA-HQ training curves  (k={history.get('topk_k', '?')})",
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Train TopK SAE on CelebA-HQ")
    parser.add_argument("--config",          type=str,   default="")
    parser.add_argument("--data-root",       type=str,   default=d.get("data_root",       "./data/celeba_hq"))
    parser.add_argument("--output-dir",      type=str,   default=o.get("output_dir",      "./outputs/sae_topk_celeba_hq_r18"))
...
def main() -> None:
    ...
    print(
        "Training setup (TopK SAE CelebA-HQ):\n"
