#!/usr/bin/env python3
"""Train BottleneckTopKMultiTaxonAutoencoder on CelebA-HQ.
...
def save_training_curves(history, output_dir) -> None:
    ...
    plt.suptitle("Training curves (Bottleneck TopK Multi-Taxon AE)", fontweight="bold")
    plt.tight_layout()
...
def parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Train Bottleneck TopK Multi-Taxon AE")
    p.add_argument("--config", type=str, default="")
    # data
    p.add_argument("--data-root", type=str, default=d.get("data_root", "./data/celeba_hq"))
    p.add_argument("--output-dir", type=str,
                   default=o.get("output_dir", "./outputs/bottleneck_topk_multi_taxon_ae_celeba_hq"))
...
def main() -> None:
    ...
    print(
        "Bottleneck TopK Multi-Taxon AE setup:\n"
        ...
        f"  per-pixel active channels (target) = K * L = {K * L} of latent {model.encoder.final_channels}\n"
