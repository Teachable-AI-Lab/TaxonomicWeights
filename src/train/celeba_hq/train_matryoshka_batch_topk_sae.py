#!/usr/bin/env python3
"""Train MatryoshkaBatchTopKSparseConvAutoencoder on CelebA-HQ.

Matryoshka Batch-TopK SAE training:
  - A single encoder + decoder are trained simultaneously at multiple
    sparsity levels (k_values, descending).
  - The **Batch-TopK** activation selects a shared global threshold so the
    model can allocate more features to complex regions.
  - The **Matryoshka loss** is a weighted sum of per-level MSEs:
        total = Σ_i loss_weights[i] * MSE(recon_i, x)
              + sparsity_weight * aux_loss
  - The AuxK auxiliary loss is computed at the largest k to prevent dead
    latent features.

Output directory suffix: _k<k1>-<k2>-..._sw<sparsity_weight>
  e.g.  sae_matryoshka_batch_topk_celeba_hq_r18_k8-4-2_sw1e-02/
"""
...
def save_training_curves(history: dict, output_dir: Path) -> None:
    ...
    plt.suptitle(
        f"Matryoshka Batch-TopK SAE CelebA-HQ  (k={k_str})",
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Train Matryoshka Batch-TopK SAE on CelebA-HQ"
    )
    parser.add_argument("--config",          type=str,   default="")
    parser.add_argument("--data-root",       type=str,   default=d.get("data_root",       "./data/celeba_hq"))
    parser.add_argument("--output-dir",      type=str,   default=o.get("output_dir",      "./outputs/celeba_hq/sae_matryoshka_batch_topk_celeba_hq_r18"))
...
def main() -> None:
    ...
    print(
        "Training setup (Matryoshka Batch-TopK SAE CelebA-HQ):\n"
