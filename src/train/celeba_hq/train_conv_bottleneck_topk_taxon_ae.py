#!/usr/bin/env python3
"""Train ConvBottleneckTopKTaxonAutoencoder on CelebA-HQ.

Strided-conv encoder/decoder (no ResNet backbone) with a hierarchical
pairwise-softmax + TopK bottleneck and AuxK dead-node revival.

Loss = recon_mse + auxk_weight * auxk + dkl_weight * dkl
"""
...
def run_validation(model, loader, device, auxk_w, dkl_w=0.0) -> dict:
    ...
    for images, _ in loader:
        ...
        dkl = float(details.get("dkl", images.new_zeros(())).item()) if dkl_w > 0 else 0.0
        loss = recon_loss + auxk_w * auxk_loss + dkl_w * dkl
        tot_loss += float(loss); tot_recon += float(recon_loss)
        tot_auxk += float(auxk_loss); tot_dead += float(dead_frac)
        tot_dkl += dkl; nb += 1
    if nb == 0:
...
def save_training_curves(history, output_dir) -> None:
    ...
    plt.suptitle("Conv Bottleneck TopK Taxon AE")
    plt.tight_layout()
...
def parse_args():
    ...
    p.add_argument("--output-dir",  type=str,   default=o.get("output_dir", "./outputs/conv_bottleneck_topk_taxon_ae_celeba_hq"))
...
def main() -> None:
    ...
    model = ConvBottleneckTopKTaxonAutoencoder(
        ...
        bottleneck_n_blocks=int(mc.get("bottleneck_n_blocks", 1)),
        topk_k_multiplier=float(mc.get("topk_k_multiplier", 1.0)),
        k_aux=mc.get("k_aux", None),
        dead_steps=int(mc.get("dead_steps", 2000)),
        temperature=float(mc.get("temperature", 0.5)),
        hard=bool(mc.get("hard", False)),
        depth_decay=float(mc.get("depth_decay", 0.5)),
        ...
        k_leaves=int(mc.get("k_leaves", 0)),
        use_gate_value=bool(mc.get("use_gate_value", False)),
    ).to(device)
    ...
    taxon_ch = model.taxon_channels
    n_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Conv Bottleneck TopK Taxon AE: device={device} n_params={n_params:,} L={L} "
          f"taxon_ch={taxon_ch} batch={args.batch_size} epochs={args.epochs} output={output_dir}")

    history = {"epochs": [],
               "train_loss": [], "train_recon": [], "train_auxk": [],
               "train_dead": [], "train_dkl": [],
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        running = {k: 0.0 for k in ("loss", "recon", "auxk", "dead", "dkl")}
        nb = 0
        for batch_idx, (images, _) in enumerate(train_loader, start=1):
            ...
            recon_loss = F.mse_loss(recon, images)
            auxk_loss = model.compute_auxk_loss(images, recon)
            dkl = details.get("dkl", images.new_zeros(()))
            ...
            if batch_idx % 50 == 0:
                ...
                print(f"epoch={epoch} batch={batch_idx}/{len(train_loader)} step={global_step} "
                      ...
                      f"auxk={avg['auxk']:.5f} dead={avg['dead']:.3f} dkl={avg['dkl']:.5f}")
            if args.max_train_steps > 0 and global_step >= args.max_train_steps:
                break

        train_stats = {k: v / max(1, nb) for k, v in running.items()}
        val_stats = run_validation(model, val_loader, device, args.auxk_weight, args.dkl_weight)
