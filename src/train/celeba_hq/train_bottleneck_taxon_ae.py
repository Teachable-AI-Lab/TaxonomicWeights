#!/usr/bin/env python3
"""Train BottleneckTaxonAutoencoder on CelebA-HQ.

Plain ResNet stages followed by a single (softmax+DKL) taxon stage at the
bottleneck.  Loss = recon + dkl_weight*dkl + entropy_weight*entropy.
"""
...
@torch.no_grad()
def run_validation(model, loader, device, dkl_weight, entropy_weight, hard) -> dict:
    model.eval()
    tot_loss = tot_recon = tot_dkl = tot_ent = 0.0
    nb = 0
...
def save_training_curves(history, output_dir) -> None:
    ...
    plt.suptitle("Bottleneck Taxon AE")
    plt.tight_layout()
...
def parse_args():
    ...
    p.add_argument("--output-dir", type=str,
                   default=o.get("output_dir", "./outputs/bottleneck_taxon_ae_celeba_hq"))
...
def main() -> None:
    ...
    print(f"Bottleneck Taxon AE setup: device={device} n_params={n_params:,} L={L} "
          f"latent_ch={model.encoder.final_channels} batch={args.batch_size} epochs={args.epochs} "
          f"output={output_dir}")
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        running = {k: 0.0 for k in ("loss", "recon", "dkl", "entropy")}
        nb = 0
        for batch_idx, (images, _) in enumerate(train_loader, start=1):
            ...
            running["dkl"] += float(dkl.item()); running["entropy"] += float(entropy.item())
            nb += 1; global_step += 1
            if batch_idx % 50 == 0:
        ...
        val_stats = run_validation(model, val_loader, device,
                                   args.dkl_weight, args.entropy_weight, args.hard)
