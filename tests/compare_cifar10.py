#!/usr/bin/env python3
"""Final comparative analysis: all CIFAR-10 model runs.

Discovers every run directory under ``outputs/`` that looks like a CIFAR-10
experiment (name contains ``cifar``), loads its best checkpoint and any
...
If no CIFAR-10 outputs exist yet the script exits gracefully with a message.
For any run without pre-computed npz files the metrics are computed live.

Output: ``outputs/comparison_cifar10/comparison_cifar10_overview.png``
...
def _short_name(run_dir: str) -> str:
    n = run_dir
    for prefix in ("taxon_ae_cifar10_r18_", "sae_cifar10_r18_",
                   "taxon_ae_cifar_r18_",   "sae_cifar_r18_",
...
def discover_runs(outputs_dir: Path) -> List[Dict]:
    ...
    runs.sort(key=lambda r: ({"taxon": 0, "sae": 1, "baseline": 2}.get(r["type"], 9), r["name"]))
...
def make_page1_reconstruction(
    ...
    plt.suptitle("Reconstruction Quality — CIFAR-10", fontsize=13, fontweight="bold")
    ...
    fig_r.suptitle("Sample Reconstructions — CIFAR-10",
...
def make_page2_sparsity(runs, all_metrics, save_dir) -> None:
    ...
    _bar_chart(axes[0, 1],
               [m["mean_l0"] for m in all_metrics], labels, colours,
    ...
    plt.suptitle("Sparsity Analysis — CIFAR-10", fontsize=13, fontweight="bold")
...
def make_page3_feature_quality(runs, all_metrics, save_dir) -> None:
    ...
    plt.suptitle("Feature Quality — CIFAR-10", fontsize=12, fontweight="bold")
    _add_legend(fig, runs)
...
def main() -> None:
    ...
    if not runs:
        print("No CIFAR-10 runs with a best.pt found under", outputs_dir)
        print("Run train_sae_cifar10.py (or train_taxon_cifar10.py) first.")
        return

    save_dir.mkdir(parents=True, exist_ok=True)
    print(f"Found {len(runs)} CIFAR-10 run(s):")
    for r in runs:
        print(f"  [{r['type']:5s}] {r['name']}")

    # Build CIFAR-10 test loader
    cifar = CIFAR10Loader(batch_size=args.batch_size, root=args.data_root)
    _, test_loader = cifar.get_loaders()
    ...
    if existing:
        ...
        combined.save(save_dir / "comparison_cifar10_overview.png")
        print(f"\nCombined overview saved to {save_dir}/comparison_cifar10_overview.png")
